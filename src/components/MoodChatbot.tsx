...
const GameAssistant: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col h-full">
      ...
      <div className="flex-grow overflow-y-auto p-4 space-y-4 h-[400px]">
        <div className="space-y-3">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`mood-chat-bubble ${
                message.sender === 'user' ? 'user-chat-bubble' : 'bot-chat-bubble'
              }`}
            >
              {message.text}
        ...
        {recommendations.length > 0 && (
          <div className="bg-game-soft/30 p-3 rounded-lg">
            <h3 className="font-medium text-sm mb-2">Recommended Games:</h3>
            <div className="grid grid-cols-1 gap-2">
              {recommendations.map((rec) => (
                <div key={rec.id} className="bg-white p-2 rounded border border-game-soft flex justify-between items-center">
                  <div>
